...
public class RaceCompleteListener {
    private final ProfileService profileService;

    @KafkaListener(topics = Constants.PROFILE_PROGRESS, groupId = Constants.KAFKA_GROUP_ID)
    public void onProcessMessage(RaceFinishEvent raceFinishEvent) {
        log.info("Получено событие успешного завершения заезда");

        profileService.progressIncrement(UUID.fromString(raceFinishEvent.getProfileId()),
